...
results will contain a list of lists (one per tweet) of triples, each triple
represents (term, type, confidence)
"""
import subprocess
import shlex
import sys
# The only relavent source I've found is here:
# http://m1ked.com/post/12304626776/pos-tagger-for-twitter-successfully
# -implemented-in
...
RUN_TAGGER_CMD = "java -XX:ParallelGCThreads=2 -Xmx500m -jar " \
                 "/Users/cocosa/Documents/tools/tweetNLP/ark-tweet-nlp-0.3.2" \
...
def _call_runtagger(tweets, run_tagger_cmd=RUN_TAGGER_CMD):
  ...
  # force UTF-8 encoding (from internal unicode type) to avoid .communicate
  # encoding error as per:
  # http://stackoverflow.com/questions/3040101/python-encoding-for-pipe
  # -communicate
  ...
  args.append('conll')
  po = subprocess.Popen(args, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
  ...
  # format\nTokenized and tagged 1 tweets (2 tokens) in 7.5 seconds: 0.1
  # tweets/sec, 0.3 tokens/sec\n')
  ...
  pos_results = [pr.split('\n') for pr in
                 pos_result]  # split parts of message by each carriage return
  return pos_results


def runtagger_parse(tweets, run_tagger_cmd=RUN_TAGGER_CMD):
  """Call runTagger.sh on a list of tweets, parse the result, return lists
  of tuples of (term, type, confidence)"""
...
def check_script_is_present(run_tagger_cmd=RUN_TAGGER_CMD):
  ...
  try:
    ...
    print "Caught an OSError, have you specified the correct path to " \
          "runTagger.sh? We are using \"%s\". Exception: %r" % (
    run_tagger_cmd, repr(err))
  return success


if __name__ == "__main__":
  print "Checking that we can see \"%s\", this will crash if we can't" % (
  RUN_TAGGER_CMD)
